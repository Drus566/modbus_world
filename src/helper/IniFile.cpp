...
IniFile::IniFile(const std::string& path, bool sync) : m_file(), m_sync(false), m_data() { load(path, sync); }
/** @brief Destructor */
IniFile::~IniFile() { }

bool IniFile::isOpen() { return m_file != ""; }
...
bool IniFile::store() const {
    bool ret = false;
    if (!m_file.empty()) ret = store(m_file);
    return ret;
}
...
void IniFile::clear() { m_data.clear(); }
...
IniFile::Value IniFile::get(const std::string& section, const std::string& name, const Value& default_value) const {
    ...
    if (it_section != m_data.end()) {
        ...
        if (it_param != it_section->second.end()) ret = Value(it_param->second);
    }
...
void IniFile::set(const std::string& section, const std::string& name, const Value& value) {
    ...
    if (it_section != m_data.end()) {
        auto it_param = it_section->second.find(name);
        if (it_param != it_section->second.end()) it_param->second = value;
        else it_section->second[name] = value;
    }
    else {
...
bool IniFile::Value::operator==(const std::string& value) const { return (m_value == value); }
...
bool IniFile::Value::isEmpty() const { return m_value.empty(); }
...
bool IniFile::Value::isBool() const { return ((m_value == "true") || (m_value == "false")); }

/** @brief Get the value as a string */
std::string IniFile::Value::toString() const { return m_value; }

/** @brief Get the value as an integer */
int IniFile::Value::toInt() const { return std::strtol(m_value.c_str(), nullptr, 10); }

/** @brief Get the value as an unsigned integer */
unsigned int IniFile::Value::toUInt() const { return std::strtoul(m_value.c_str(), nullptr, 10); }

/** @brief Get the value as a floating point value */
double IniFile::Value::toFloat() const { return std::strtod(m_value.c_str(), nullptr); }

/** @brief Get the value as a boolean */
bool IniFile::Value::toBool() const { return ((m_value == "true") ? true : false); }
