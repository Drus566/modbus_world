...
bool Map::readWord(WORD adr, WORD *val, MemMode mode) {
	std::lock_guard<std::mutex> lock(m_mtx);
...
bool Map::readDWord(WORD adr, DWORD *val, MemMode mode) {
	std::lock_guard<std::mutex> lock(m_mtx);
...
bool Map::readWords(WORD adr, WORD quantity, WORD *val, MemMode mode) {
	std::lock_guard<std::mutex> lock(m_mtx);
...
bool Map::writeWord(WORD adr, WORD *val, MemMode mode) {
	std::lock_guard<std::mutex> lock(m_mtx);
	if (adr < m_start_adr || adr > m_end_adr || val == nullptr) return false;
	if (mode == MemMode::LITTLE_ENDIAN_BYTE_SWAP_MODE);
	WORD offset = adr - m_start_adr;
...
bool Map::writeDWord(WORD adr, DWORD *val, MemMode mode) {
	std::lock_guard<std::mutex> lock(m_mtx);
	if (adr < m_start_adr || val == nullptr || m_end_adr < adr + 1) return false;
	if (mode == MemMode::LITTLE_ENDIAN_BYTE_SWAP_MODE);
	WORD offset = adr - m_start_adr;
...
bool Map::writeWords(WORD adr, WORD quantity, WORD *val, MemMode mode) {
	std::lock_guard<std::mutex> lock(m_mtx);
	if (adr < m_start_adr || val == nullptr || quantity == 0 || m_end_adr < adr + quantity - 1) return false;
	if (mode == MemMode::LITTLE_ENDIAN_BYTE_SWAP_MODE);
	WORD offset = adr - m_start_adr;
...
bool Map::readBit(WORD adr, BIT *val, MemMode mode) {
	std::lock_guard<std::mutex> lock(m_mtx);
...
bool Map::readBits(WORD adr, WORD quantity, BIT *val, MemMode mode) {
	std::lock_guard<std::mutex> lock(m_mtx);
