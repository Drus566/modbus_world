...
int Serial::connect() {
	...
	if (m_debug) {
		...
		<< (int)m_data_bit << ", " << m_stop_bit << ")" << std::endl;
	}
	...
	if (m_fd < 0) {
		if (m_debug) {
			std::cout << "Error open device " << m_device << " (" << strerror(errno) << ")";
		}
		return -1;
	}
	...
	else speed = getTermiosSpeed(m_baudrate);
...
long Serial::doReceive(uint8_t* rsp, int rsp_length) {
...
int Serial::waitReceive(uint8_t *msg) {
	int rc;
	fd_set rset;
	struct timeval tv;
	struct timeval *p_tv;
	...
	while (length_to_read != 0) {
		rc = doSelect(&rset, p_tv, length_to_read);
		if (rc == -1) {
...
int Serial::flush() { return tcflush(m_fd, TCIOFLUSH); }

int Serial::doSelect(fd_set* rset, timeval* tv, int length_to_read) {
	int s_rc;
...
static speed_t getTermiosSpeed(int baud, int debug) {
	speed_t speed;
